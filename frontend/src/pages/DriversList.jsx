...
import { SearchIcon, PlusIcon, EditIcon, TrashIcon, PhoneIcon, CarIcon, BikeIcon, Eye, ChevronLeft, ChevronRight } from 'lucide-react';
...
const DriversList = () => {
    ...
    }, [activeTab, page, searchTerm, filterVehicle]);
    ...
    const fetchData = async () => {
        try {
            ...
            // Paramètres de requête pour le serveur
            const params = {
                page,
                limit: 10,
                search: searchTerm,
                vehicle: filterVehicle !== 'all' ? filterVehicle : undefined
            };

            // Utiliser les endpoints appropriés selon l'onglet
            switch (activeTab) {
                case 'active':
                    driversRes = await driverService.getActive(params);
                    break;
                case 'former':
                    driversRes = await driverService.getFormer(params);
                    break;
                default:
                    driversRes = await driverService.getAll(params);
            }

            // Charger les véhicules séparément
            const vehiclesRes = await vehicleService.getAll();

            // Log de débogage
            console.log('Réponse des véhicules:', vehiclesRes);
            console.log('Type de vehiclesRes.data:', typeof vehiclesRes.data);

            // Forcer un tableau de véhicules
            const vehiclesData = Array.isArray(vehiclesRes.data)
                ? vehiclesRes.data
                : (vehiclesRes.data?.vehicles || vehiclesRes.data?.data || []);

            setDrivers(driversRes.data.drivers || []);
            setTotalPages(driversRes.data.totalPages || 1);
            setTotalDrivers(driversRes.data.totalCount || 0);
