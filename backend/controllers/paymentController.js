...
exports.create = async (req, res) => {
    ...
    try {
        ...
        await History.create({
            ...
            description: `Création d'un paiement de ${amount}FCFA pour le planning ${scheduleId.substring(0, 8)}`,
            ipAddress: req.ip
        });
...
exports.update = async (req, res) => {
    ...
    try {
        ...
        await History.create({
            ...
            performedBy: req.user?._id,
            description: `Mise à jour du paiement $${payment._id.toString().substring(0, 8)}
 de ${existingPayment.amount}FCFA → ${payment.amount}FCFA`,
            ipAddress: req.ip
        });
...
exports.deletePhoto = async (req, res) => {
    try {
        ...
        await History.create({
            ...
            performedBy: req.user?._id,
            description: `Suppression d'une photo du paiement $${payment._id.toString().substring(0, 8)}
}
}`,
            ipAddress: req.ip
        });
...
exports.delete = async (req, res) => {
    ...
    try {
        ...
        await History.create({
            ...
            performedBy: req.user?._id,
            description: `Suppression du paiement ${payment._id.toString().substring(0, 8)} de ${payment.amount}FCfA pour le planning ${scheduleId.substring(0, 8)}`,
            ipAddress: req.ip
        });
...
exports.changeStatus = async (req, res) => {
    ...
    try {
        ...
        await History.create({
            ...
            performedBy: req.user?._id,
            description: `Changement de statut du paiement ${payment._id.toString().substring(0, 8)} (${existingPayment.status} → ${payment.status})`,
            ipAddress: req.ip
        });
...
exports.getDriverStats = async (req, res) => {
    ...
    try {
        ...
        for (const driver of drivers) {
            ...
            console.log(`[PaymentController] getDriverStats - Statistiques calculées pour le chauffeur ${driver._id}: ${totalPayments} paiements, ${totalAmount}FCFA, ${averageAmount}FCFA par paiement, ${targetMet} paiements cibles, ${targetMetPercentage}% total`);
